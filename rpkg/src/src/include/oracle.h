...
template <typename T> struct PenaltyL0 {
  const T l0;
...
template <typename T> struct PenaltyL1 {
  const T l1;
...
template <typename T> struct PenaltyL2 {
  const T l2;
...
struct NoBounds {
  ...
  static bool inline validate() { return true; }
};
...
template <class P, class B> struct Oracle {
  const P penalty;
  const B bounds;
