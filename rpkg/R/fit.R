...
gL0Learn.gfit <- function(x, # nolint
  ...
  if (gL0Learn.is.real_scalar(l0) && gL0Learn.is.real_scalar(l2)) {
    ...
    gL0Learn.is.real_matrix(l2, c(p, p))) {
    if (!(gL0Learn.is.real_scalar(l1) ||
      gL0Learn.is.real_matrix(l0, c(p, p)))) {
  ...
  if (gL0Learn.is.real_scalar(lows) && gL0Learn.is.real_scalar(highs)) {
    if (lows != -Inf || highs != Inf) {
      ...
      stop("exected lows to be less than or equal to zero and
             highs to be greater than or equal to 0")
    }
  } else if (!(gL0Learn.is.real_matrix(lows, c(p, p)) &&
    gL0Learn.is.real_matrix(highs, c(p, p)))) {
    stop("expected that lows or highs be scalar values
           or matricies of dims (p, p).")
  }
