...
test_that("Active set difference works as expected", {
    larger <- 1:10
    smaller <- 1:3
    expect_equal(test_sorted_vector_difference(larger, smaller), 4:10)
    expect_equal(test_sorted_vector_difference2(larger, smaller), 4:10)
    
    smaller <- 1:10
    expect_equal(length(test_sorted_vector_difference(larger, smaller)), 0)
    expect_equal(length(test_sorted_vector_difference2(larger, smaller)), 0)
    
    smaller <- 5:10
    expect_equal(test_sorted_vector_difference(larger, smaller), 1:4)
    expect_equal(test_sorted_vector_difference2(larger, smaller), 1:4)
    
    smaller <- seq(1, 10, 2)
    expect_equal(test_sorted_vector_difference(larger, smaller), seq(2, 10, 2))
    expect_equal(test_sorted_vector_difference2(larger, smaller), seq(2, 10, 2))
    
    smaller <- integer(0)
    expect_equal(test_sorted_vector_difference(larger, smaller), larger)
...
test_that("Active set insert works as expected", {
    ...
    x1 <- 1:10
    x2 <- seq(1, 10, by=2)
    expect_equal(test_insert_sorted_vector_into_sorted_vector(x1, x2),
...
test_that('check_is_valid_coordinate_subset works as expected', {
    for (i in 1:10){
        expect_true(check_is_valid_coordinate_subset(upper_triangluar_coords(i+1), 
                                                     upper_triangluar_coords(i)))
        expect_true(check_is_valid_coordinate_subset(upper_triangluar_coords(i), 
                                                     upper_triangluar_coords(i)))
    }
    
    for (i in 3:10){
        expect_false(check_is_valid_coordinate_subset(upper_triangluar_coords(i-1),
                                                      upper_triangluar_coords(i)))
    }
