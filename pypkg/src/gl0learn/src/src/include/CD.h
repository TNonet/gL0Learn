...
struct CDParams {
  ...
  CDParams(const double tol, const size_t max_active_set_size,
      ...
      : tol{tol},
        max_active_set_size{max_active_set_size},
        gap_method{gap_method},
        one_normalize{one_normalize},
        max_iter{max_iter},
        oracle{oracle},
...
class CD {
 public:
  CD(const TY &Y, const TT &theta, const TP &params,
     ...
     const coordinate_vector &super_active_set)
      : Y{Y},
        S_diag{arma::sum(arma::square(Y), 0)},
        params{params},
        super_active_set{super_active_set} {
    this->theta = TT(theta);  // Take a copy of theta!
    this->active_set =
        coordinate_vector(active_set);  // Ensure a copy is taken.
  ...
  std::tuple<coordinate_vector, std::vector<double> > active_set_expansion(
 ...
 private:
  ...
  std::vector<double> costs;  // TODO(TNonet): Should be a map of where this
                              // value was from. {"initial_objective": X, "CD_1"
...
fitmodel CD<TY, TR, TT, TP>::fit() {
  ...
  while (cur_iter <= this->params.max_iter) {
    ...
    double old_objective = cur_objective;
...
CD<TY, TR, TT, TP>::active_set_expansion(
  ...
  for (; it != it_end; ++it) {
    ...
    const auto a_ij = this->a(i, j);
    const auto b_ij = this->b(i, j);

    const double q_ij = this->params.oracle.Q(a_ij, b_ij, i, j);
...
void CD<TY, TR, TT, TP>::inner_fit() {
  ...
  for (; it != it_end; ++it) {
    ...
    const auto a_ij = this->a(i, j);
    const auto b_ij = this->b_update(i, j);

    const double new_theta = this->params.oracle.Q(a_ij, b_ij, i, j);
...
fitmodel CD<TY, TR, TT, TP>::fitpsi() {
  ...
  for (size_t i = 0; i < this->params.max_swaps; i++) {
    ...
    for (; it != it_end; ++it) {
      ...
      // TODO(TNonet): This needs to properly update the active set.
...
template <class TY, class TR, class TT, class TP>
std::tuple<bool, arma::uword> CD<TY, TR, TT, TP>::psi_row_fit(
    const arma::uword row_ix) {
  // TODO(TNonet): Check if this can be {row_ix, row_ix+1}
  // TODO(TNonet): Check if we should be storing active_set in column major sort
  // order?
  const coordinate row_ix_0 = {row_ix, 0};
  auto it = std::lower_bound(this->super_active_set.begin(),
                             this->super_active_set.end(), row_ix_0);

  // TODO(TNonet): Check if this can be {row_ix+1, row_ix+2}
  // TODO(Tnonet): Check if we should be storing active_set in column major sort
  // order?
