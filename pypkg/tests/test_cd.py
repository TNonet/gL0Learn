...
def test_super_active_set(algorithm, p, module, overlaps, lXs):
    ...
    possible_active_set = np.asarray(np.where(np.abs(np.triu(test_result.theta, k=1)) > 0)).T
...
def test_cd_vs_mosek(n, p, module, overlaps, lXs):
    ...
    penalty = Penalty(**lXs)
    MIO_loss = pseudo_likelihood_loss(y, np.array(MIO_results.theta_hat), penalty, active_set=MIO_active_set)
    cd_loss = pseudo_likelihood_loss(y, np.array(cd_results.theta), penalty, active_set=CD_active_set)
...
def test_cd_keeps_mio_results(max_iter, algorithm, n, p, module, overlaps, lXs):
    ...
    try:
        ...
        MIO_loss = pseudo_likelihood_loss(y, np.array(MIO_results.theta_hat), penalty, active_set=active_set)
        cd_loss = pseudo_likelihood_loss(y, np.array(cd_results.theta), penalty, active_set=active_set)
